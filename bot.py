...
status = cycle(['Python', 'доту с онлином','аниме!!!'])
...
async def on_command_error(ctx, err):
    if isinstance(err, discord.ext.commands.errors.CommandNotFound):
        await ctx.reply(embed=discord.Embed(title=f"Ошибка!", description=f"Команда **не найден!**", color=0xFF0000))

    elif isinstance(err, discord.ext.commands.errors.MissingPermissions):
        await ctx.reply(embed=discord.Embed(title=f"Ошибка!",
...
async def unban(ctx, id: int):
    ...
    await ctx.reply(embed=unbanned)
...
async def prefix(ctx, *, prefixsetup=None):
    ...
    await ctx.reply(embed=discord.Embed(title="Готово!", description=f"Префикс изменён на > ``{prefixsetup}`` "
...
async def restart(ctx):
    ...
    await ctx.reply(embed=restarted)
    restart_bot()
